...
PERMANENT FIX — url.py -> config.json seeding
Every startup, _seed_urls_from_url_py() bridges compile-time url.py constants
into the runtime config.json so sync_service always has URLs to call.

STATIC MODE  (DYNAMIC_API_ENABLED = False in url.py):
  All url_* keys overwritten on EVERY startup. Deleting config.json is safe.

DYNAMIC MODE (DYNAMIC_API_ENABLED = True in url.py):
  One-time seed when all URL fields are blank. User-saved values persist.
"""
...
class ConfigManager:
    def __init__(self):
        ...
        self.config_dir = Path(_local_appdata) / "EnterpriseMonitor"
