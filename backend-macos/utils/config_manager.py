...
Storage path: ~/Library/Application Support/EnterpriseMonitor/config.json
...
═══════════════════════════════════════
See backend-windows/utils/config_manager.py for the full explanation.
Short version: sync_service._get_url() reads from config.json. If config.json
has empty strings (every fresh install), sync is permanently skipped even when
BASE_URL is set in url.py.  _seed_urls_from_url_py() bridges the two at startup.
"""
...
class ConfigManager:
    ...
    def _seed_urls_from_url_py(self) -> None:
        ...
        Dynamic mode (DYNAMIC_API_ENABLED = True):
          ...
          populate them from BASE_URL + PATH_*.  Once the user saves their own
          values via the GUI, those values persist and url.py is ignored.
        """
        try:
            from url import (
                DYNAMIC_API_ENABLED,
