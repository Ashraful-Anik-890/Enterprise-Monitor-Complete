"""
keylogger.py — macOS version
Captures typed text by buffering keystrokes and flushing on ENTER or window switch.

Uses pynput.keyboard.Listener — runs its own internal thread with NSRunLoop,
so this class is non-blocking by design. The public API matches the other
monitoring services (start / stop / pause / resume).
...
- Removed win32gui, win32process imports
- _get_active_window_info() uses osascript JXA subprocess
- Removed sys.platform != "win32" guard — pynput is valid on macOS
- pynput.keyboard.Listener manages its own NSRunLoop on macOS — safe
...
def _get_active_window_info():
    ...
    macOS version using osascript JXA subprocess. Safe from any thread.
...
class Keylogger:
    ...
    The pynput Listener runs its own daemon thread internally (with NSRunLoop
    on macOS). This class only wraps lifecycle management around it.
    """
    ...
    def start(self):
        ...
        # Log a warning about Input Monitoring — it may be silently denied
        logger.info(
            "Keylogger starting. If Input Monitoring permission is denied in "
            "System Settings → Privacy & Security → Input Monitoring, pynput "
            "will start but receive zero events."
        )
